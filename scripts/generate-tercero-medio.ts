...
/**
 * Generación completa de pools para 3° Medio
 */
...
async function getCompletedPools(): Promise<Set<string>> {
    ...
    try {
        const response = await r2Client.send(new ListObjectsV2Command({
            ...
            Prefix: 'exercises/tercero-medio/',
        }));

        const completed = new Set<string>();
        response.Contents?.forEach(item => {
            const name = item.Key?.replace('exercises/tercero-medio/', '').replace('.json', '');
            if (name && name !== 'test') {
                completed.add(name);
            }
        });
...
async function generatePoolForSubject(
    ...
    const tracker = new ProgressTracker(
        ...
        `🎯 Generando pool de ejercicios: ${subjectName} (3° Medio)`,
        EXERCISES_PER_POOL,
        { gradeId, subjectId, subjectName, grade: '3° Medio' }
    );
...
async function generateTerceroMedio() {
    console.log('\n' + '='.repeat(70));
    console.log('🔄 GENERACIÓN DE POOLS - 3° MEDIO');
    console.log('='.repeat(70) + '\n');

    const terceroMedio = curriculum.find(g => g.id === 'tercero-medio');
    if (!terceroMedio) {
        console.error('❌ No se encontró 3° Medio');
        return;
    }
    ...
    const pending = terceroMedio.subjects.filter(s => !completed.has(s.id));
    ...
    for (let i = 0; i < pending.length; i++) {
        ...
        const success = await generatePoolForSubject(
            ...
            subject.id,
            subject.name,
            i + 1,  // Número de la materia actual en la lista de pendientes
            terceroMedio.subjects.length,  // Total de materias
            completed.size  // Materias ya completadas
        );
    ...
    console.log(`📦 Total pools: ${completed.size + completedNow}/${terceroMedio.subjects.length}`);
    console.log('='.repeat(70) + '\n');
}
