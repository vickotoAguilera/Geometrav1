...
/**
 * Generación completa de pools para 2° Medio
 */
...
async function getCompletedPools(): Promise<Set<string>> {
    ...
    try {
        const response = await r2Client.send(new ListObjectsV2Command({
            ...
            Prefix: 'exercises/segundo-medio/',
        }));

        const completed = new Set<string>();
        response.Contents?.forEach(item => {
            const name = item.Key?.replace('exercises/segundo-medio/', '').replace('.json', '');
            if (name && name !== 'test') {
                completed.add(name);
            }
        });
...
async function generatePoolForSubject(
    ...
    const tracker = new ProgressTracker(
        ...
        `🎯 Generando pool de ejercicios: ${subjectName} (2° Medio)`,
        EXERCISES_PER_POOL,
        { gradeId, subjectId, subjectName, grade: '2° Medio' }
    );
...
async function generateSegundoMedio() {
    console.log('\n' + '='.repeat(70));
    console.log('🔄 GENERACIÓN DE POOLS - 2° MEDIO');
    console.log('='.repeat(70) + '\n');

    const segundoMedio = curriculum.find(g => g.id === 'segundo-medio');
    if (!segundoMedio) {
        console.error('❌ No se encontró 2° Medio');
        return;
    }
    ...
    const pending = segundoMedio.subjects.filter(s => !completed.has(s.id));
    ...
    console.log(`📦 Total pools: ${completed.size + completedNow}/${segundoMedio.subjects.length}`);
    console.log('='.repeat(70) + '\n');
}
