...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   🚀 INICIAR GENERACIÓN 3° MEDIO EN SEGUNDO PLANO            ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
if [ -f "$PID_FILE" ]; then
    ...
    if ps -p $PID > /dev/null 2>&1; then
        ...
        echo "   Usa './stop-tercero-medio.sh' para detenerlo primero"
        exit 1
    else
...
cd "$PROJECT_DIR"
nohup npx tsx scripts/generate-tercero-medio.ts >> "$LOG_FILE" 2>&1 &
PID=$!
...
echo "   - Ver progreso:  ./status-tercero-medio.sh"
echo "   - Ver log:       tail -f $LOG_FILE"
echo "   - Pausar:        ./pause-tercero-medio.sh"
echo "   - Detener:       ./stop-tercero-medio.sh"
echo ""
