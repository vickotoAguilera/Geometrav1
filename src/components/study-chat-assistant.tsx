  ...
  return (
    <div className="text-sm p-3 rounded-lg bg-secondary text-secondary-foreground w-full space-y-3">
      <p className="font-medium">
        ¡Hola! Soy tu tutor para este tema. Mi conocimiento se limita a este documento.
      </p>
      <p>
        Pídeme ayuda con un ejercicio o concepto específico.
      </p>
      <Button
        size="sm"
        variant="outline"
...
export function StudyChatAssistant({ studyMaterial, topicTitle }: { studyMaterial: string, topicTitle: string }) {
  ...
  const sendInitialPrompt = () => {
    ...
    const initialPrompt = `Estoy viendo el tema '${topicTitle}'. Ayúdame a entenderlo.`;
    handlePromptClick(initialPrompt);
    ...
    <>
      ...
      <ScrollArea className="flex-1" viewportRef={viewportRef}>
        <div className="p-4 space-y-6">
          {messages.length === 0 ? (
            <WelcomeMessage onHelpClick={sendInitialPrompt} />
          ) : (
